...
class UploaderTableModel extends AbstractTableModel {
    ...
    private def filesAndUploadStatuses = [:]
    ...
    def getUploadStatus(int row) {
        return filesAndUploadStatuses[fileList[row]]
    }
    ...
    void handleUploadEvent(BaseEvent evt) {
        ...
        } else if (evt instanceof DataUploadedEvent) {

            DataUploadedEvent e = (DataUploadedEvent)evt
            fileName = e.fileName
            def upStatus = filesAndUploadStatuses[fileName]
            if (!upStatus) {
                upStatus = [:]
                filesAndUploadStatuses[fileName] = upStatus
            }
            upStatus.max = e.totalSize
            upStatus.value = e.uploaded
